...
public class CategoryService {
    ...
    public List<CategoryResponseDTO> getByType(CategoryType type) {
        ...
        return categoryRepository.findByUserIdAndTypeOrderByIdAsc(user.getId(), type)
                .stream()
                .map(this::toDTO)
                .toList();
    }
    ...
    @Transactional(readOnly = true)
    public CategoryGroupedDTO getAll() {
        User user = authUtil.getCurrentUser();
        List<Category> all = categoryRepository.findByUserIdOrderByIdAsc(user.getId());

        List<CategoryResponseDTO> expenses = all.stream()
                .filter(c -> c.getType() == CategoryType.EXPENSE)
                .map(this::toDTO)
                .collect(Collectors.toList());

        List<CategoryResponseDTO> incomes = all.stream()
                .filter(c -> c.getType() == CategoryType.INCOME)
                .map(this::toDTO)
                .collect(Collectors.toList());

        return new CategoryGroupedDTO(expenses, incomes);
    }
